...
def check_git_status():
    ...
    LOGGER.info(emojis(s))  # emoji-safe  自动更新代码
...
def check_dataset(data, autodownload=True):
    ...
    # Resolve paths 解析路径
    path = Path(extract_dir or data.get('path') or '')  # optional 'path' default to '.'  默认根目录
...
def colorstr(*input):
    ...
    *args, string = input if len(input) > 1 else ('blue', 'bold', input[0])  # color arguments, string
    colors = {'black': '\033[30m',  # basic colors
              ...
              'bold': '\033[1m',
              'underline': '\033[4m'}
    return ''.join(colors[x] for x in args) + f'{string}' + colors['end']#自定义颜色字体'end'
...
def labels_to_class_weights(labels, nc=80):
    ...
    classes = labels[:, 0].astype(int)  # labels = [class xywh]      #np.int--int🚀
...
def labels_to_image_weights(labels, nc=80, class_weights=np.ones(80)):
    ...
    class_counts = np.array([np.bincount(x[:, 0].astype(int), minlength=nc) for x in labels])     #np.int --int🚀
...
def coco80_to_coco91_class():  # converts 80-index (val2014) to 91-index (paper)
    # https://tech.amikelive.com/node-718/what-object-categories-labels-are-in-coco-dataset/ coco数据集
...
def xyxy2xywhn(x, w=640, h=640, clip=False, eps=0.0):#eps裁剪大小
...
def segments2boxes(segments):
    ...
    return xyxy2xywh(np.array(boxes))  # cls, xywh 坐标转换
...
def resample_segments(segments, n=1000):
    ...
    for i, s in enumerate(segments):
        ...
        xp = np.arange(len(s))
        segments[i] = np.concatenate([interp(x, xp, s[:, i]) for i in range(2)]).reshape(2, -1).T  # segment xy     np.interp--interp🚀
    return segments
...
def strip_optimizer(f='best.pt', s=''):  # from utils.general import *; strip_optimizer() 导入
...
def feature_visualization(features, model_type, model_id, feature_num=64):
    ...
    """
    save_dir = "runs/detect"             #  "features/"   保存路径
