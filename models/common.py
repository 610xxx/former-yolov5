...
import torch.nn.functional as F  #添加c3str模块
...
#注意力模块nam，抑制不显著的特征权重🚀，轻量级类似cbam，但无外加参数 理论上减少计算量
...
#NAM只考虑轻量级别，性能不如cbam,不适合车牌检测，进准度比cbam低
...
#添加simam注意力模块🚀提高connets表征能力 类似se模块，但无需额外参数
...
class C3Ghost(C3):
    # C3 module with GhostBottleneck()  聚集
    def __init__(self, c1, c2, n=1, shortcut=True, g=1, e=0.5):
        super().__init__(c1, c2, n, shortcut, g, e)
        c_ = int(c2 * e)  # hidden channels
...
# ECA注意力机制之一：引用卷积，计算效率高，准确性同时少计算量，存在部分误差
class ECA(nn.Module):
...
"""
#类似c3tr,容易造成内存爆炸
#添加c3str机制--提高模型稳定性，从原始低分辨率转换高分辨率🚀🚀🚀🚀🚀🚀🚀🚀🚀🚀
class SwinTransformerBlock(nn.Module):
    def __init__(self, c1, c2, num_heads, num_layers, window_size=8):
        super().__init__()
        self.conv = None
