...
def export_onnx(model, im, file, opset, train, dynamic, simplify, prefix=colorstr('ONNX:')):
    ...
    try:
        ...
        torch.onnx.export(model, im, f, verbose=False, opset_version=opset,
                          ...
                          dynamic_axes={'images': {0: 'batch_size', 2:'height',   3:'width' },  # shape(1,3,640,640)             
                                        'output': {0: 'batch_size', 1: 'anchors' }  # shape(1,25200,85)             ⛵⛵⛵⛵⛵
...
def run(data=ROOT / 'data/coco128.yaml',  # 'dataset.yaml path'
    ...
    return f  # return list of exported files/dirs   70432  736 32 768


def parse_opt():
    parser = argparse.ArgumentParser()
    parser.add_argument('--data', type=str, default=ROOT / 'data/coco128.yaml', help='dataset.yaml path')
    parser.add_argument('--weights', nargs='+', type=str, default=ROOT / 'runs/train/exp120/weights/best.pt', help='model.pt path(s)')#'yolov5s.pt'   1152/1408.        120国内脱敏      256国内doodle
    parser.add_argument('--imgsz', '--img', '--img-size', nargs='+', type=int, default=[800, 800], help='image (h, w)')#640.960.1024.1280.1312.64倍数
    ...
    parser.add_argument('--dynamic', action='store_true',default=False, help='ONNX/TF: dynamic axes')#动态尺寸导出
    parser.add_argument('--simplify', action='store_true',default=False, help='ONNX: simplify model')#简化模型 （True可能导致应用端口精度下降）(onnxsim)
    ...
    parser.add_argument('--iou-thres', type=float, default=0.15, help='TF.js NMS: IoU threshold')#0.0  0.45  （越高，可能框越多）   
    parser.add_argument('--conf-thres', type=float, default=0.2, help='TF.js NMS: confidence threshold')#0.0 0.25 置信度值
